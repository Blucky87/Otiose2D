...
namespace Nez.SpriterAnimator
{
  public class FrameDataCalculator
  {
    ...
    public virtual FrameData GetFrameData(SpriterAnimation first, SpriterAnimation second, float targetTime,
      float deltaTime, float factor)
    {
      FrameData.Clear();
      ...
      if (boneInfosA != null && boneInfosB != null)
      {
        ...
        for (int i = 0; i < boneInfosA.Length; ++i)
        {
          ...
          interpolated.Angle = Mathf.CloserAngleLinear(boneA.Angle, boneB.Angle, factor);
      ...
      for (int i = 0; i < baseKey.ObjectRefs.Length; ++i)
      {
        ...
        info.Angle = Mathf.CloserAngleLinear(interpolatedFirst.Angle, interpolatedSecond.Angle, factor);
        info.PivotX = Mathf.Linear(interpolatedFirst.PivotX, interpolatedSecond.PivotX, factor);
        info.PivotY = Mathf.Linear(interpolatedFirst.PivotY, interpolatedSecond.PivotY, factor);

        if (boneInfos != null && objectRefFirst.ParentId >= 0)
          info.ApplyParentTransform(boneInfos[objectRefFirst.ParentId]);

        AddSpatialData(info, currentAnimation.Timelines[objectRefFirst.TimelineId], currentAnimation.Entity.Spriter,
          deltaTime);
    ...
    public virtual FrameData GetFrameData(SpriterAnimation animation, float targetTime, float deltaTime,
      SpriterSpatial parentInfo = null)
    {
    ...
    protected virtual void UpdateMetadata(SpriterAnimation animation, float targetTime, float deltaTime,
      SpriterSpatial parentInfo = null)
    {
      ...
      for (int i = 0; i < animation.Timelines.Length; ++i)
      {
        ...
        if (Config.VarsEnabled && meta.Varlines != null && meta.Varlines.Length > 0)
        {
          for (int j = 0; j < timeline.Meta.Varlines.Length; ++j)
          {
            ...
            FrameData.AddObjectVar(objInfo.Name, variable.Name,
    ...
    protected virtual SpriterVarValue GetVariableValue(SpriterAnimation animation, SpriterVarDef varDef,
      SpriterVarline varline, float targetTime)
    {
      ...
      float adjustedTime = keyA.Time == keyB.Time
        ? targetTime
        : SpriterHelper.AdjustTime(targetTime, keyA, keyB, animation.Length);
    ...
    protected virtual SpriterSpatial[] GetBoneInfos(SpriterMainlineKey key, SpriterAnimation animation, float targetTime,
      SpriterSpatial parentInfo = null)
    {
    ...
    protected virtual void GetMainlineKeys(SpriterMainlineKey[] keys, float targetTime, out SpriterMainlineKey keyA,
      out SpriterMainlineKey keyB)
    {
