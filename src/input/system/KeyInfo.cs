...
namespace Otiose.Input
{
    public struct KeyInfo
    {
        ...
        private readonly Keys[] keyCodes;


        private KeyInfo(Keys key, string name, params Keys[] keyCodes)
        {
            this.key = key;
            this.name = name;
            this.macName = name;
        ...
        private KeyInfo(Keys key, string name, string macName, params Keys[] keyCodes)
        {
            this.key = key;
            this.name = name;
        ...
        public bool IsPressed
        {
            get
            {
                var keyCodeCount = Enum.GetNames(typeof(Keys)).Length;
                var keys = Enum.GetValues(typeof(Keys));
                for (int i = 0; i < keyCodeCount; i++)
                {
                    if (Nez.Input.isKeyDown(key))
                    {
                        return true; 
                    }
                }
        ...
        public static readonly KeyInfo[] KeyList = {
            new KeyInfo(Keys.None, "None", Keys.None ),

			new KeyInfo(Keys.Left, "Left", Keys.Left ),
			new KeyInfo(Keys.Right, "Right", Keys.Right ),
			new KeyInfo(Keys.Up, "Up", Keys.Up ),
			new KeyInfo(Keys.Down, "Down", Keys.Down),
/*
